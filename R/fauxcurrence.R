...
#' @param iter.max.stg1 The maximum number of iterations with no improvement before stage 1 is restarted.
#' @param iter.max.stg2 As with iter.max.stg1 but for stage 2.
#' @param iter.max.stg3 The maximum number of iterations for the the stage 3 iterative improvement procedure before the algorithm is stopped if div.n.flat is not reached.
...
#' }
#' @export
fauxcurrence <-function(coords,rast,distmat=NULL,use.distmat=TRUE,inter.spp=FALSE,sep.inter.spp=FALSE,fix.seed.pts=NULL,div.n.flat=1000,allow.ident.conspec=FALSE,dist_meth="distRcpp",dist_fun="Haversine",trim.init.range=TRUE,init.range=0.9,new.pt.meth.stg.2="dist.dens",new.pt.meth.stg.3="dist.dens",switch.n=1,iter.max.stg1=10000,iter.max.stg2=2000,iter.max.stg3=100000,div.int=10,break.num=20,ret.seed.pts=FALSE,ret.all.iter=FALSE,logfile=NULL,verbose=TRUE) {
  # record start time
  start_time <- Sys.time()
  ...
  if(verbose){
    cat("Starting stage 1: initial seed point generation...")
    stg.1.start <- Sys.time()
  }
  ######## STAGE 1
  ...
  if(is.null(fix.seed.pts)){
    ...
    repeat{
      ...
      if(iter.unchanged > iter.max.stg1){
        ...
        cnt.wrong.old <- NULL
        iter.unchanged <- 0
        if(verbose) cat(paste("\n","maximum iterations of",iter.max.stg1,"reached, restarting seed point generation. If this happens several times, consider adjusting iter.max.stg1..."))
      }
      pts.new <- pts
  ...
  if(verbose){
    stg.1.time <- difftime(Sys.time(),stg.1.start,units="secs")
    cat(paste("completed in",round(as.numeric(stg.1.time),2),units(stg.1.time),"\n"))
  }
  ######## STAGE 1 END
  ...
  if(verbose){
    cat("Starting stage 2: full seed point generation...")
    stg.2.start <- Sys.time()
  }
  ...
  # keep initial seed points incase the loop gets stuck
  pts.seed <- pts
  pts.species.seed <- pts.species
  iter.unchanged <- 0
  ########  STAGE 2
  ...
  while(length(which(is.na(pts.species))) > 0){
    # if maximum iterations for stage 2 has been reached, restart from initial seed points
    if(iter.unchanged > iter.max.stg2){
      pts <- pts.seed
      pts.species <- pts.species.seed
      if(verbose) cat(paste("\n","maximum iterations of",iter.max.stg2,"reached, restarting seed point generation. If this happens several times, consider adjusting iter.max.stg2..."))
    }
  ...
  if(verbose){
    stg.2.time <- difftime(Sys.time(),stg.2.start,units="secs")
    cat(paste("completed in",round(as.numeric(stg.2.time),2),units(stg.2.time),"\n"))
  }
  ######## STAGE 2 END
  ...
  if(verbose){
    ...
    cat(paste("completed in",round(as.numeric(stg.3.time),2),units(stg.3.time),"with",L-1,"iterations","\n\n"))
  ...
  if(verbose){
    ...
    cat(paste("Total analysis completed in",round(as.numeric(total.time),2),units(total.time),"\n"))
  }
  return(out)
} # end
